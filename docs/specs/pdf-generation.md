...
Decksmith's PDF generator is a **worker-based async system** that produces high-quality, customizable proxy sheets. Users configure paper format, grid layout, margins, cut lines, and DPI before generating.
...
function drawCutLines(doc: PDFDocument, pos: Position, config: CutLineConfig) {
  ...
  // Top-left
  doc.moveTo(pos.x, pos.y - markLength).lineTo(pos.x, pos.y).stroke()
  doc.moveTo(pos.x - markLength, pos.y).lineTo(pos.x, pos.y).stroke()

  // Top-right
  doc.moveTo(pos.x + pos.width, pos.y - markLength).lineTo(pos.x + pos.width, pos.y).stroke()
  doc.moveTo(pos.x + pos.width, pos.y).lineTo(pos.x + pos.width + markLength, pos.y).stroke()

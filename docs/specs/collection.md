...
Manage your Magic: The Gathering card inventory with specific editions, variants, conditions,
pricing, and a 3D card viewer.
...
**As a collector, I want to add a specific card print to my collection so I can track my
inventory.**
...
**As a user importing my collection from another app, I want to upload a CSV so I don't manually
enter 1000+ cards.**
...
**As a user with a large collection, I want to customize how I view my inventory so I can find cards
quickly.**
...
**Table View:** | Column | Description | Sortable | Filterable |
|--------|-------------|----------|------------| | Image | Thumbnail | No | No | | Name | Card name
| Yes | Yes (autocomplete) | | Set | Set code + icon | Yes | Yes (multi-select) | | Quantity | #
copies | Yes | Yes (range) | | Condition | NM/LP/MP/HP/DMG | Yes | Yes (multi-select) | | Foil | ✓
or ✗ | Yes | Yes (checkbox) | | Price | USD or EUR | Yes | Yes (range slider) | | Tags | User tags
(pills) | No | Yes (multi-select) | | Custom Fields | User-defined (JSONB) | No | No |
...
**As a collector, I want to add custom metadata to cards so I can track physical location and
acquisition details.**
...
**As a user, I want to inspect cards in 3D with foil effects so I can appreciate artwork and verify
details.**
...
3. **WebGL fallback**
   ...
   - If unsupported: Show static 2D image + message "Your browser doesn't support 3D viewer. Upgrade
     to a modern browser."
...
export function CardViewer3D({ cardPrint, isFoil }) {
  return (
    <Canvas camera={{ position: [0, 0, 150] }}>
      ...
      <CardMesh imageUrl={cardPrint.image_uris.large} isFoil={isFoil} />
      <OrbitControls maxPolarAngle={Math.PI / 3} minPolarAngle={Math.PI / 3} />
    </Canvas>
  );
}
```
...
```json
{
  "imported": 98,
  "skipped": 2,
  "errors": [{ "row": 42, "reason": "Card not found: set=XYZ, number=999" }]
}
```
...
- **"View Full Stats" button**: Opens modal with charts (mana curve, color distribution, set
  breakdown)
...
**3-column grid** (between mobile 2-col and desktop 4-col) **Side panel** for filters (not bottom
sheet)
...
- [ADR-0009: Responsive Feature Strategy](../adr/0009-responsive-feature-strategy.md) — Grid layout,
  filter patterns

...
**Why it's wrong:** Error messages from external SDKs can change between versions. This check
can break silently.
...
**Why it's wrong:** `codes.ts` contains our **public API error codes** — the contract returned
to clients, used as i18n keys. Supabase error codes are infrastructure implementation details.

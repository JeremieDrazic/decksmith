...
healthy. `SyncState` observability (`lastCheckedAt` + `startedAt`) on branch
`feat/sync-state-heartbeat` (PR #116 open, `db:push` applied). Remaining in 3.3: the endpoints. This
file describes the **current** state only: environment, what works today, blockers, and what's next.
...
- **Phase 3.3 endpoints** — `GET /cards/search` (+ `CardSearchQuerySchema`: filters/sort/pagination)
  - `/cards/:id` + `/cards/:id/prints` + autocomplete; Postgres indexing (generated `tsvector` +
    ...
    4.3). **Next up.**
- **Merge #116** (`SyncState` heartbeat) — additive + green; merge → deploy → worker runs with the
...
- **#116** `feat/sync-state-heartbeat` — `SyncState.lastCheckedAt` + `startedAt` observability
  (`db:push` applied). Additive + green; not yet merged.
- #114 (Phase 3.3 sync levels 1 & 2) and #115 (bulk-upsert OOM fix, closes #96) **merged**.
...
- `feat/sync-state-heartbeat` — `SyncState.lastCheckedAt` + `startedAt` observability. Based on
  `main` post-#115. **Pushed — PR #116 open.** Next: merge #116, then the Phase 3.3 endpoints.
